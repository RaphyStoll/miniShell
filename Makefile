...
ifeq ($(USE_ANIM), yes)
	...
	count=$$(echo $$colors | wc -w); \
	rand_index=$$(awk -v n=$$count 'BEGIN{srand(); print int(1 + rand() * n)}'); \
	color=$$(echo $$colors | cut -d' ' -f$$rand_index); \
	...
	@printf "██║ ╚═╝ ██║██║██║   ██║██║███████║██║██║ ╚███║███████╗███████╗███████╗\n"
	@printf "╚═╝     ╚═╝╚═╝╚═╝   ╚═╝╚═╝╚══════╝╚═╝╚═╝  ╚══╝╚══════╝╚══════╝╚══════╝\n"
	@printf "$(RESET)\n$(GREEN)✨ Minishell built successfully!$(RESET)\n"
else
	@clear
	@printf "$(BOLD)$(CYAN)\n"
	@printf "███╗   ███╗██╗███╗███╗██╗███████╗██╗███╗  ██╗███████╗██╗     ██╗     \n"
